...
import { readCodexHostIndex } from '../sdk/codex-host-index.mjs';
import { CodexAgent } from '../sdk/codex-agent.mjs';
import { historyEvent } from '../sdk/codex-history.mjs';
...
test('host index includes archived and legacy records and never modifies the source database', async () => {
  ...
  try {
    ...
    if (path.dirname(dir) === os.tmpdir() && path.basename(dir).startsWith('xiaomeng-host-test-')) await rm(dir, { recursive: true, force: true });
  }
});

test('tool history keeps text while excluding binary image payloads and metadata', () => {
  const event = historyEvent('thread', 'turn', { id: 'tool', type: 'mcpToolCall', tool: 'capture',
    result: { content: [{ type: 'text', text: 'Screen captured' }, { type: 'image', data: 'binary-payload' }],
...
test('history preserves reply phase and separates tool input from its result', () => {
  const progress = historyEvent('thread', 'turn', { id: 'progress', type: 'agentMessage', text: 'Checking files', phase: 'commentary' });
  assert.equal(progress.phase, 'commentary');
  assert.equal(progress.turn_id, 'turn');
  assert.equal(progress.item_id, 'progress');
  assert.equal(historyEvent('thread', 'turn', { id: 'reply', type: 'agentMessage', text: 'Done', phase: 'final_answer' }).phase, 'final_answer');
  const tool = historyEvent('thread', 'turn', { id: 'tool', type: 'mcpToolCall', arguments: { title: 'Inspect page' }, result: { content: [{ type: 'text', text: 'Page ready' }] } });
...
test('live item and text deltas preserve the same phase and identity as history', async () => {
  ...
  const adapter = new CodexAgent({ rpc: new EventEmitter(), projects: [cwd],
    client: { isAvailable: () => true, event: async (session, type, fields) => published.push({ type, ...fields }) } });
  adapter.threads.set('thread', { id: 'thread', cwd });
  await adapter.item('thread', 'turn', { id: 'reply', type: 'agentMessage', phase: 'commentary', text: '' }, false);
  await adapter.notification({ method: 'item/agentMessage/delta', params: { threadId: 'thread', turnId: 'turn', itemId: 'reply', delta: 'Checking files' } });
  await adapter.item('thread', 'turn', { id: 'reply', type: 'agentMessage', phase: 'commentary', text: 'Checking files' }, true);
  assert.deepEqual(published.map((event) => [event.type, event.phase, event.turnId, event.itemId]), [
    ['assistant.delta', 'commentary', 'turn', 'reply'], ['message.assistant', 'commentary', 'turn', 'reply'],
  ]);
});

test('unfinished host turns are read-only until attached to the executing server', async () => {
  const published = [];
  const adapter = new CodexAgent({ rpc: new EventEmitter(), projects: [path.resolve('host-project')],
    client: { isAvailable: () => true, request: async (_, body) => published.push(body) } });
  await adapter.publish({ id: 'active', cwd: path.resolve('host-project'), lastTurnStatus: 'inProgress' });
...
test('host discovery follows every page, includes archived/legacy tasks, and publishes only changes', async () => {
  ...
  const summaries = Array.from({ length: 135 }, (_, i) => ({ id: `thread-${i}`, cwd: project, name: `Task ${i}`, updatedAt: 1000 + i }));
  const rpc = Object.assign(new EventEmitter(), { request: async (method, params) => {
    calls.push([method, params]);
    assert.equal(method, 'thread/list');
    if (params.archived) return { data: [{ id: 'archive', cwd: project, updatedAt: 1000 }], nextCursor: null };
    return params.cursor ? { data: summaries.slice(100), nextCursor: null } : { data: summaries.slice(0, 100), nextCursor: 'next' };
  } });
  const client = { isAvailable: () => true, project: async () => {}, request: async (_, body) => published.push(body) };
  const adapter = new CodexAgent({ rpc, client, projects: [], hostScope: true, lazyHistory: true,
    hostIndex: async () => ({ projects: [{ id: 'saved', cwd: project, name: 'Saved project' }],
...
test('history pages preserve cursor and stable item identities without resuming a task', async () => {
  ...
  const rpc = Object.assign(new EventEmitter(), { request: async (method, params) => {
    calls.push([method, params]);
    return { data: [{ turnId: 'turn', item: { id: 'output', type: 'agentMessage', text: 'Full output' } }], nextCursor: 'older' };
  } });
  const project = path.resolve('host-project');
  const adapter = new CodexAgent({ rpc, client: { isAvailable: () => true }, projects: [project] });
  ...
  assert.equal(page.events[0].event_key, historyEvent('thread', 'turn', { id: 'output', type: 'agentMessage', text: 'Full output' }).event_key);
});
