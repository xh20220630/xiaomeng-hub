/** 封装 Claude CLI 的任务调度和流式输出，避免同一会话并发写入。 */
...
import { sessionInfo } from './transcript.js';
import { broadcast } from '../../events/hub-events.js';
import { sessionSettings } from '../../services/settings.service.js';

/** 记录用户取消，避免将进程终止误报为普通失败。 */
type ClaudeChild = ChildProcessWithoutNullStreams & {
  /** 用户主动停止标记，避免误报普通执行失败。 */
  __aborted?: boolean;
};
...
const children = new Map<string, ClaudeChild>(); // sessionId -> ChildProcess
const queues = new Map<string, Promise<void>>(); // sessionId -> tail Promise (serial queue)

const BASE_ARGS = ['--output-format', 'stream-json', '--include-partial-messages', '--verbose'];

/**
 * 等待进程真正启动后才交给调用方，并保留启动错误用于回退。
 * @param cmd 不经过 shell 拼接的可执行程序。
 * @param args 固定命令行参数或主机动作参数。
 * @param opts 子进程启动配置。
 * @returns 已启动的 Claude 子进程。
 */
function spawnOnce(
  cmd: string,
  args: string[],
...
/**
 * 按平台尝试可执行入口，用户指令始终留在 stdin。
 * @param args 固定命令行参数或主机动作参数。
 * @param cwd 主机执行任务的工作目录。
 * @returns 成功启动的 Claude 子进程。
 */
async function spawnClaude(args: string[], cwd: string): Promise<ClaudeChild> {
  ...
  for (const [cmd, a] of candidates) {
    try {
      ...
      if (
        e &&
        (e.code === 'EINVAL' || e.code === 'ENOENT' || e.code === 'EACCES' || e.code === 'UNKNOWN')
      ) {
...
/**
 * 优先消费 token 增量，避免随后完整助手消息造成重复文本。
 * @returns 有独立流状态的文本提取函数。
 */
function makeExtractor() {
  let sawDelta = false;
  return (ev: TranscriptRecord) => {
 ...
 * @param task 按会话排队执行的任务。
 * @returns 本次工作项的结果。
 */
function enqueue(key: string, task: () => Promise<ClaudeRunResult>): Promise<ClaudeRunResult> {
...
/**
 * 管理 CLI 输入、文本流及结束结果，并关联真实会话 ID。
 * @param options 本次操作的具名参数，缺省值由实现统一处理。
 * @param options.args 固定命令行参数或主机动作参数。
 * @param options.cwd 主机执行任务的工作目录。
 * @param options.text 用户指令、输出或待处理文本。
 * @param options.requestKey 调用方的幂等请求键。
 * @param options.onInit 主机确认会话标识时的回调。
 * @param options.onDelta 每次收到增量文本时的回调。
 * @returns 进程结果和最终累计文本。
 */
async function runClaude({
  ...
  try {
    ...
    return {
      ok: false,
      text: '',
      sessionId: requestKey || null,
      error: `无法启动 claude CLI: ${e.message}`,
    };
  }
  const keys = new Set<string>();
  /**
   * 把已知会话 ID 关联到子进程，以支持针对会话停止。
   * @param id 待处理实体的稳定标识。
   * @returns 无返回值。
   */
  const track = (id: string | undefined | null) => {
    if (id) {
      keys.add(id);
    ...
    child.on('close', (code) => {
      ...
      resolve({
        ...
        error: ok
          ...
          : aborted
            ? '已停止'
            : stderr.trim().slice(0, 400) || `claude 退出码 ${code}`,
      });
    });
  });
}
...
/**
 * 按会话串行续聊，并把下一轮设置作为固定 CLI 参数传入。
 * @param sessionId 目标会话标识，使用当前协议一侧的命名空间。
 * @param cwd 主机执行任务的工作目录。
 * @param text 用户指令、输出或待处理文本。
 * @param options 本次操作的具名参数，缺省值由实现统一处理。
 * @param options.onDelta 每次收到增量文本时的回调。
 * @param options.onDone 轮次结束时接收完整文本的回调。
 * @returns 执行结果和累计回复。
 */
export function sendMessageStream(
  ...
  text: string,
  { onDelta, onDone }: ClaudeCallbacks = {},
) {
 ...
 * @returns 新会话的执行结果。
 */
export function startSession(cwd: string, text: string, { onInit, onDelta }: ClaudeCallbacks = {}) {
 ...
 * @returns 本次发送的执行结果。
 */
export async function sendToSession(sessionId: string, text: string) {
  ...
  console.log(
    `[send] resume ${String(sessionId).slice(0, 8)} (cwd=${info?.cwd || '?'}) len=${String(text).length}`,
  );
...
/**
 * 仅停止本平台持有的进程；Windows 同时终止子进程树。
 * @param sessionId 目标会话标识，使用当前协议一侧的命名空间。
 * @returns 是否找到并发出了停止请求。
 */
